...
export default function Navigation() {
  ...
  const boxShadow = useMotionTemplate`0 4px 20px rgba(0, 0, 0, ${shadowOpacity})`;
  ...
  useEffect(() => {
    ...
    // Debounce: only fire after 50ms of scroll inactivity
  ...
  const scrollToSection = (href: string) => {
    if (href.startsWith('/')) {
      // Navigate to a different page — Next.js way (instant, no reload!)
      router.push(href);
    } else {
